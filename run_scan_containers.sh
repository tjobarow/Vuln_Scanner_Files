...
docker run --restart=always -d --name=full-fast-scan -v /home/scanuser/gvm-data/reports:/reports/:rw thedoctor0/openvas-docker-lite python3 -u scan.py $full_cidr_subnet --update -o=full_fast_$date.xml >> /home/scanuser/gvm-data/logs/run_scan_containers.log
...
docker run --restart=always -d --name=system-disc-scan -v /home/scanuser/gvm-data/reports:/reports/:rw thedoctor0/openvas-docker-lite python3 -u scan.py $full_cidr_subnet -p="System Discovery" --update -o=system_discovery_$date.xmli >> /home/scanuser/gvm-data/logs/run_scan_containers.log
